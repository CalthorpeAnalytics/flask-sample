...
class APISpecExt:
    """Very simple extension to use apispec with this API as a flask extension"""

    def __init__(self, app: Optional[Flask] = None, **kwargs: Any):
        self.spec = None
    ...
    def init_app(self, app: Flask, **kwargs: Any) -> None:
        app.config.setdefault("APISPEC_TITLE", "geoeditor")
    ...
    def swagger_json(self) -> str:
        if not self.spec:
            raise Exception("APISpecExt has not been initialized.")

        return jsonify(self.spec.to_dict())

    def swagger_ui(self) -> Text:
